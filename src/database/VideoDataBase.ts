...
export class VideosDataBase extends BaseDatabase{
    ...
    public  async findVideos(q: string | undefined){
        ...
        if (q) {
            const result: TVideoDB[] = await BaseDatabase.connection(VideosDataBase.TABLE_VIDEOS).where("name", "LIKE", `%${q}%`)
            videosDB = result
        } else {
            const result: TVideoDB[] = await BaseDatabase.connection(VideosDataBase.TABLE_VIDEOS)
            videosDB = result
        }
        return videosDB
